...
La valeur `restUrl` peut être utilisée pour que l’[API REST](https://dev.outlook.com/restapi/reference) appelle la boîte aux lettres de l’utilisateur.
...
En mode composition, vous devez appeler la méthode [`saveAsync`](Office.context.mailbox.item#saveAsync) avant de pouvoir utiliser le membre `restUrl`. Votre application doit disposer des autorisations **ReadWriteItem** pour appeler la méthode `saveAsync`.

> **Remarque :** Les clients Outlook connectés aux installations locales d’Exchange 2016 avec une URL REST personnalisée configurée renvoient une valeur non valide pour `restUrl`.
...
|---|---|---|---|
| `eventType` | [Office.EventType](Office.md#EventType) || Événement qui doit appeler le gestionnaire. |
...
> **Remarque :** les compléments devraient, dans la mesure du possible, utiliser les API REST à la place des services web Exchange. 

**Jetons REST**

Quand un jeton REST est demandé (`options.isRest = true`), le jeton fourni ne permet pas d’authentifier les appels des services web Exchange. Le jeton peut uniquement accéder en lecture seule à l’élément actif et à ses pièces jointes, sauf si l’autorisation [`ReadWriteMailbox`](../../docs/add-ins/outlook/understanding-outlook-add-in-permissions#readwritemailbox-permission) est spécifiée dans le manifeste du complément. Si l’autorisation `ReadWriteMailbox` est spécifiée, le jeton fourni accorde un accès en lecture/écriture au courrier, au calendrier et aux contacts, ainsi que la possibilité d’envoyer des messages.
...
|`callback`| fonction||Une fois la méthode exécutée, la fonction transmise au paramètre `callback` est appelée avec un seul paramètre, `asyncResult`, qui est un objet [`AsyncResult`](simple-types.md#asyncresult). Le jeton est fourni sous forme de chaîne dans la propriété `asyncResult.value`.|
