...
# <a name="document.getselecteddataasync-method"></a>Méthode Document.getSelectedDataAsync
...
|**Paramètre _coercionType_ spécifié**|**Données renvoyées**|**Prise en charge de l’application hôte Office**|
|:-----|:-----|:-----|
...
|**Office.CoercionType.Matrix** ou `"matrix"`|Tableau de tableaux. Par exemple, ` [['a','b'], ['c','d']]` pour une sélection de deux lignes dans deux colonnes.|Word et Excel.|
|**Office.CoercionType.Table** ou `"table"`|Objet [TableData](../../reference/shared/tabledata.md) pour la lecture d’un tableau avec des en-têtes.|Word et Excel.|
...
Pour lire la valeur de la sélection, vous devez écrire une fonction de rappel qui le fasse.L’exemple suivant montre comment procéder :
