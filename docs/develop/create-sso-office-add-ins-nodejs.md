...
1. Un **URI d’ID d’application** a été généré sous la forme « api://{application ID GUID} ». Remplacez le GUID par « localhost:3000 ». L’ID dans son intégralité doit indiquer `api://localhost:3000`. (La partie domaine du nom d’**étendue**, juste en dessous de l’**URI d’ID d’application** change automatiquement en conséquence. Il doit apparaître comme suit : `api://localhost:3000/access_as_user`.)

1. Cette étape et la suivante permettent à l’application hôte Office d’accéder à votre complément. La section **Applications préalablement autorisées** contient une zone **ID d’application** vide. Entrez l’ID suivant dans la zone (il s’agit de l’ID de Microsoft Office) : `d3590ed6-52b3-4102-aeff-aad2292ab01c`.

1. Ouvrez le menu déroulant **Étendue** à côté de l’**ID d’application** et activez la case à cocher `api://localhost:3000/access_as_user`.
...
3. Dans la chaîne affectée à la propriété `issuer`, vous verrez l’espace réservé *{O365 tenant GUID}*. Remplacez ceci par l’ID du client Office 365 que vous avez enregistré à la fin de la dernière procédure. Si pour une raison quelconque, vous n’avez pas obtenu l’ID antérieur, utilisez l’une des méthodes de la page [Trouver votre ID de client Office 365](https://support.office.com/en-us/article/Find-your-Office-365-tenant-ID-6891b561-a52d-4ade-9f39-b492285e2c9b) pour l’obtenir. Lorsque vous avez terminé, la valeur de propriété `issuer` doit ressembler à ceci :
    ...
    > **Remarque :** Conservez les autres paramètres du constructeur `AuthModule` inchangés.
...
2. Remplacez TODO2 par le code suivant. À propos de ce code, notez que :
...
3. Remplacez TODO3 par le code suivant, qui envoie la requête HTTP au point de terminaison de jeton du STS.
...
1. Toujours dans src/auth.ts, ajoutez la méthode ci-après à la classe `AuthModule`. Tenez compte des informations suivantes :
    ...
    * La méthode recherche d’abord dans le stockage permanent un jeton d’accès à la ressource qui n’a pas expiré et qui ne va pas expirer dans la minute qui suit. Il appelle la méthode que vous avez créée dans la dernière section uniquement si nécessaire.
...
5. Remplacez TODO6 par la ligne suivante. Tenez compte des informations suivantes :

    * L’appel vers `acquireTokenOnBehalfOf` ne comprend pas un paramètre de ressource étant donné que nous avons construit l’objet `AuthModule` (`auth`) avec le point de terminaison AAD V2 qui ne prend pas en charge une propriété de ressource.
    * Le deuxième paramètre de l’appel spécifie que les autorisations dont le complément aura besoin pour obtenir une liste des fichiers et dossiers de l’utilisateur dans OneDrive Entreprise.

    `const graphToken = await auth.acquireTokenOnBehalfOf(jwt, ['profile', 'Files.Read.All']);`

6. Remplacez TODO7 par la ligne suivante. Tenez compte des informations suivantes :
...
7. Remplacez TODO8 par le code suivant. Notez que Microsoft Graph renvoie des métadonnées OData et une propriété **eTag** pour chaque élément, même si `name` est la seule propriété demandée. Le code envoie uniquement les noms d’éléments au client.
...
1. Si vous n’utilisez pas Visual Studio Code : 
 2. Ouvrez un terminal de nœud et accédez au dossier racine du projet.
 3. Dans le terminal, entrez **npm run build**. 
 4. Ouvrez un second terminal de nœud et accédez au dossier racine du projet.
 5. Dans le terminal, entrez **npm run start**.

2. Si vous utilisez VS Code :
 3. Ouvrez le projet dans VS Code.
 4. Appuyez sur CTRL-MAJ-B pour générer le projet.
 5. Appuyez sur F5 pour exécuter le projet dans une session de débogage.
...
> **Remarque :** La version d’aperçu de l’API `getAccessTokenAsync` prend en charge uniquement les identités professionnelles ou scolaires (Office 365). *Si vous êtes connecté à Office avec une identité personnelle (Microsoft Account), déconnectez-vous avant de continuer.* Pour tester le complément, vous devez être entièrement déconnecté d’Office ou connecté avec un compte scolaire ou professionnel.

1. Vérifiez que vous possédez certains fichiers ou dossiers dans votre compte OneDrive Entreprise.
