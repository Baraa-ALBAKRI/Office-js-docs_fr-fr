...
Un complément Outlook contient deux composants : le manifeste du complément XML et une page web, pris en charge par la bibliothèque JavaScript pour les compléments Office (office.js). Le manifeste décrit l’intégration du complément avec les clients Outlook. Voici un exemple.
 ...
 >**Remarque**  Dans l’exemple suivant, les valeurs de toutes les URL commencent par « https://appdemo.contoso.com ». Cette valeur est un espace réservé. Dans un manifeste valide réel, ces valeurs contiennent des URL web HTTP valides.
...
|**VersionOverrides** 1.0|Prend en charge les versions ultérieures de l’API JavaScript pour Office. Les commandes de complément sont prises en charge.|
|**VersionOverrides** 1.1|Prend en charge les versions ultérieures de l’API JavaScript pour Office. Les commandes de complément sont prises en charge, ainsi que de nouvelles fonctionnalités, telles que les [volets des tâches épinglables](./pinnable-taskpane.md) et les compléments mobiles.|
...
Cet élément est également l’endroit où les compléments définissent la prise en charge de [compléments mobiles](./add-mobile-support.md).
...
> **Remarque :** Les règles d’activation s’appliquent uniquement aux clients qui ne prennent pas en charge l’élément **VersionOverrides**. 
